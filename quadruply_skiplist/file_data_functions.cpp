...
void update_employee(QuadruplySkipList* skiplist, int id , int salary, int department, int last_id){
    QuadruplySkipList_Node* p = skiplist->find(id); //find the employee if it exists
    if(id < 1 || id > last_id || p->get_data_id()!=id){  //employee does not exits if it is null
        cout << "ERROR: An invalid ID to update" <<"\n";
        return;
    }
    p->get_data()->set_salary(salary);
    p->get_data()->set_department(department);

}

void delete_employee(QuadruplySkipList* skiplist, int id, int last_id){
    if(id < 1 || id > last_id ){ //check the id is valid
