...
Node* BinaryTree::get_root(){ //get the real root
...
void BinaryTree::insert(Employee* employee){
    ...
    while(true){
        if(employee->get_id() > p->get_employee_id()){
            //the id is bigger so we continue with right
        ...
        else if(employee->get_id() < p->get_employee_id()){
            //the id is smaller so we continue with left
            ...
            p=p->get_left(); //continue searching
        }
        else{
...
void BinaryTree::preorder_traverse(Node* n){
    
    if(n!=NULL){
        cout<<n->get_employee_id()<<";"<<n->get_employee_salary()<<";"<< n->get_employee_department()<<endl; //print the parent
        this->preorder_traverse(n->get_left()); //print the left subtree
        this->preorder_traverse(n->get_right()); //print the right subtree
    }
}

void BinaryTree::printToConsole(){
    cout<< "P" << endl;
    this->preorder_traverse(root->get_right());
...
int BinaryTree::getHeight_Recursive(Node* node){
    //base case
    if(node == NULL){
        return 0;
    }
    else{
        int left_sub_tree = getHeight_Recursive(node->get_left());
        int right_sub_tree = getHeight_Recursive(node->get_right());
        //take the bigger height
...
int BinaryTree::getHeight(){
    //call the recursion function for the root
    return getHeight_Recursive(root->get_right());
}
